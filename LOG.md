...
## 2026-08-18 解析証拠データの遅延読み込み

- 起動時に読む`font-coverage-data.js`と`font-opentype-data.js`をスキーマ2の軽量サマリーへ更新した。圧縮済み`ranges`、`codepointCount`、解析状態、Weight / Style、ファイル数、版の概要、統合済みOpenType機能一覧は維持し、未収録文字判定と機能チップの表示を遅延化していない。
- ファイル別URL、SHA-256、ファイルサイズ、`unicode-range`、User-Agent、CSS取得情報、ファイルごとのOpenType機能を`analysis-details/<font-id>.js`へ分離し、cmap側とOpenType側に重複していたファイル別情報を詳細JSの1つの`files`配列へ正規化した。詳細JSにフォント本体は含めない。
- `analysis-details-loader.js`は許可済みフォントIDと相対パスの対応表だけを使用する。読込中は同じPromiseを共有し、成功後は同一画面内でキャッシュする。失敗時はキャッシュを外して再試行でき、登録データがない詳細JSと未解析フォントをエラー／詳細なしとして扱う。
- OpenType詳細ダイアログは起動時サマリーを即時表示した後、対象フォントの解析証拠だけを読み込む。読込中・成功・失敗・再試行・詳細なしを表示し、閉じる／別フォントへ切り替える際は古い非同期結果が現在の表示を上書きしない。
- `analyze_font_cmap.py`と`analyze_google_fonts.py`はサマリー2ファイルと対象詳細JSを一時出力・検証してからまとめて置換する。部分更新は他フォントのサマリーと詳細を削除しない。容量は`node scripts/measure-analysis-data.mjs`でraw/gzipを再計測できる。
