...
## 2026-08-18 OpenType機能情報のカード別折りたたみ表示

### 変更内容

- `app.js`
  - OpenType機能の共通定義を追加（タグ/名称/説明）
  - 各フォントに `attributes.openType` を追加して `収録確認済み / 未収録 / 未確認` を管理する構造を導入
  - 各フォントカードの属性欄に `OpenType機能: ...` の要約表示を追加
  - カード内で折りたたみ式 `OpenType機能` トグルを追加（`button`、`aria-expanded`、`aria-controls`、`aria-label`）
  - Webフォント `noto-sans-jp-web` を `sourceType: 'web'` で追加し、同一のカード生成・表示ロジックで扱う
- `style.css`
  - トグル、矢印アイコン、折りたたみパネル、OpenType項目のスタイルを追加
  - `prefers-reduced-motion` 下で開閉アニメーションを無効化
- `index.html`
  - Noto Sans JP の読み込みを追加（`fonts.googleapis.com`）
- `README.md`
  - OpenType機能折りたたみの使い方、未確認と未収録の区別、システム/ Web フォント共通運用を追記

### 実装上の判断

- `openType` 情報が未検証のフォントは `OpenType機能は未確認です。収録状況を確認できていません。` と表示し、未確認を未収録として断定しない。
- 展開内容はタグ名・機能名・説明・対応状態を共通説明辞書で表示し、フォントごとの重複記述を抑制。
- 未確認データがあるフォントも展開領域を空欄のままにはせず、メッセージを表示して一覧崩れを避けた。
